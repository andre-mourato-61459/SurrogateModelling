...
\subsection{Mesh morphing}
  ...
  A key challenge in morphing anatomical structures lies in the absence of distinct anatomical landmarks. To address this, we developed an automated approach for extracting pseudo-landmarks—referred to as Source Points (SPs) to guide the morphing process. These SPs were defined by uniformly sampling previously extracted centerline-based splines along the aortic wall. For each geometry, 16 SPs were selected per spline across 10 cross-sectional planes.
...
\subsection{Artificial intelligence pipeline}
  ...
  \subsubsection{Data pre-processing}
    ...
    \begin{figure}
      ...
      \caption{\gls{DNN} performance overview: summary of the NMAE and R for all test samples. Labels A--D denote representative cases with very high (A), high (B), moderate (C), and low (D) agreement levels, which are examined in detail in Figs. \ref{fig:surface_representationStress} and \ref{fig:surface_representationStrain}.}
  ...
  \subsubsection{Performance metrics}
    ...
    These metrics were estimated independently for the predicted distributions of Second Piola-Kirchhoff stress ($\boldsymbol{S}$) and Right Cauchy-Green deformation ($\boldsymbol{C}$). Additionally, the same analysis was repeated for the subset of patient-specific~\gls{ATAA} geometries. To better access the performance of the surrogate model, two thresholds for success were defined:
    \begin{itemize}
...
\begin{table}[b]
      \caption{Summary statistics of the surrogate model performance on the test set.\label{tab:tableX}}
...
\subsection{Neural network performance} 
  ...
  \subsubsection{Case 2 -- Patient-specific geometries}
    ...
    For the patient-specific geometry dataset, the success rates for high agreement changed notably: 51 \% and 64 \% for the predictions of the Second Piola-Kirchhoff stress and Right Cauchy-Green deformation, respectively. Nonetheless, the success rates for moderate agreement remained similar (95 \% and 97 \%, respectively). As with the PCA-driven cases, the~\gls{DNN} predicted spatial distributions follow the same qualitative patterns as the~\gls{CSM} simulations. However, more pronounced differences in the magnitude of the variables are observed in regions of reduced mechanical loading.
